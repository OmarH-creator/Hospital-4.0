...
public class HospitalApp extends Application {
    ...
    // --- Constants ---
    // Color scheme
    private static final String COLOR_PRIMARY = "#3498db";  // Blue
    private static final String COLOR_SECONDARY = "#2ecc71";  // Green
    private static final String COLOR_ACCENT = "#e74c3c";  // Red
    private static final String COLOR_NEUTRAL = "#ecf0f1";  // Light gray
    private static final String COLOR_DARK = "#34495e";  // Dark blue/gray
    
    // Button styles
    private static final String BUTTON_STYLE_PRIMARY = "-fx-background-color: " + COLOR_PRIMARY + 
                                                    "; -fx-text-fill: white; -fx-font-weight: bold; " + 
                                                    "-fx-padding: 10 30 10 30; -fx-font-size: 15px; " + 
                                                    "-fx-background-radius: 5;";
    
    private static final String BUTTON_STYLE_SECONDARY = "-fx-background-color: " + COLOR_SECONDARY + 
                                                     "; -fx-text-fill: white; -fx-font-weight: bold; " + 
                                                     "-fx-padding: 10 30 10 30; -fx-font-size: 15px; " + 
                                                     "-fx-background-radius: 5;";
    
    private static final String BUTTON_STYLE_VIEW = "-fx-background-color: #f8f9fa; -fx-border-color: #dee2e6; " + 
                                                 "-fx-border-radius: 4; -fx-background-radius: 4; " +
                                                 "-fx-padding: 5 10 5 10;";
    
    private static final String BUTTON_STYLE_EDIT = "-fx-background-color: #e3f2fd; -fx-border-color: " + COLOR_PRIMARY + 
                                                 "; -fx-border-radius: 4; -fx-background-radius: 4; " +
                                                 "-fx-padding: 5 10 5 10;";
    
    private static final String BUTTON_STYLE_DELETE = "-fx-background-color: #ffebee; -fx-border-color: " + COLOR_ACCENT + 
                                                   "; -fx-border-radius: 4; -fx-background-radius: 4; " +
                                                   "-fx-padding: 5 10 5 10;";
    
    private static final String BUTTON_STYLE_ACTION1 = "-fx-background-color: #e8f5e9; -fx-border-color: " + COLOR_SECONDARY + 
                                                    "; -fx-border-radius: 4; -fx-background-radius: 4; " +
                                                    "-fx-padding: 5 10 5 10;"; // Greenish (Admit, Complete)
    
    private static final String BUTTON_STYLE_ACTION2 = "-fx-background-color: #e3f2fd; -fx-border-color: " + COLOR_PRIMARY + 
                                                    "; -fx-border-radius: 4; -fx-background-radius: 4; " +
                                                    "-fx-padding: 5 10 5 10;"; // Bluish (Discharge, Cancel)
    
    private static final String BUTTON_STYLE_ACTION3 = "-fx-background-color: #fff8e1; -fx-border-color: #ffc107; " + 
                                                    "-fx-border-radius: 4; -fx-background-radius: 4; " +
                                                    "-fx-padding: 5 10 5 10;"; // Yellowish (Mark Paid)
    
    // TableView styles
    private static final String TABLE_STYLE = "-fx-background-color: white; " +
                                           "-fx-border-color: #dee2e6; " +
                                           "-fx-border-radius: 8; " +
                                           "-fx-background-radius: 8;";
    
    // Background styles
    private static final String BG_STYLE_LIGHT = "-fx-background-color: white;";
    private static final String BG_STYLE_NEUTRAL = "-fx-background-color: " + COLOR_NEUTRAL + ";";
    
    // Title styles
    private static final String TITLE_STYLE = "-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: " + COLOR_DARK + ";";
    private static final String SUBTITLE_STYLE = "-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: " + COLOR_DARK + ";";
    ...
    // --- CENTERING & FORMATTING FIX START ---
    // Utility for date/time formatting
    private static final java.time.format.DateTimeFormatter DATE_FORMAT = java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd");
    private static final java.time.format.DateTimeFormatter DATETIME_FORMAT = java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd | HH:mm");
    // --- CENTERING & FORMATTING FIX END ---

    // --- MOCK DATA START ---
    private void loadMockData() {
        try {
            // Patients
            Patient p1 = new Patient("P101", "Alice Smith", LocalDate.of(1990, 5, 12));
            Patient p2 = new Patient("P102", "Bob Johnson", LocalDate.of(1985, 8, 23));
            Patient p3 = new Patient("P103", "Charlie Lee", LocalDate.of(1978, 2, 3));
            controller.registerPatient(p1);
            controller.registerPatient(p2);
            controller.registerPatient(p3);

            // Inventory Items
            InventoryItem i1 = new InventoryItem("INV101", "Aspirin", 50, 0.25);
            InventoryItem i2 = new InventoryItem("INV102", "Bandage", 100, 0.10);
            InventoryItem i3 = new InventoryItem("INV103", "Stethoscope", 5, 25.00);
            controller.addInventoryItem(i1);
            controller.addInventoryItem(i2);
            controller.addInventoryItem(i3);

            // Appointments
            Appointment a1 = new Appointment("A1001", p1, LocalDateTime.now().plusDays(1).withHour(10).withMinute(0), "Checkup");
            Appointment a2 = new Appointment("A1002", p2, LocalDateTime.now().plusDays(2).withHour(14).withMinute(30), "Consultation");
            controller.scheduleAppointment(a1);
            controller.scheduleAppointment(a2);

            // Medical Records
            MedicalRecord r1 = new MedicalRecord("MR10001", p1, a1, "Flu", "Prescribed rest and fluids.", LocalDate.now());
            MedicalRecord r2 = new MedicalRecord("MR10002", p2, a2, "Sprained Ankle", "Applied bandage. Advised ice and elevation.", LocalDate.now());
            controller.addMedicalRecord(r1);
            controller.addMedicalRecord(r2);

            // Bill
            Bill b1 = new Bill("B101", p1);
            b1.addLineItem(new Bill.LineItem("Consultation Fee", 50.00));
            controller.createBill(b1);
        } catch (Exception e) {
            System.err.println("MOCK DATA ERROR: " + e.getMessage());
        }
    }
    // --- MOCK DATA END ---
    ...
    private static final String RECORDS_FILE = DATA_DIR + "/medical_records.txt";
    ...
    private void loadAllData() {
        loadPatients();
        // --- LOAD TO SERVICE FIX START ---
        for (Patient p : patientList) controller.registerPatient(p);
        // --- LOAD TO SERVICE FIX END ---
        loadInventory();
        // --- LOAD TO SERVICE FIX START ---
        for (InventoryItem i : inventoryList) controller.addInventoryItem(i);
        // --- LOAD TO SERVICE FIX END ---
        loadAppointments();
        // --- LOAD TO SERVICE FIX START ---
        for (Appointment a : appointmentList) controller.scheduleAppointment(a);
        // --- LOAD TO SERVICE FIX END ---
        loadMedicalRecords();
        // --- LOAD TO SERVICE FIX START ---
        for (MedicalRecord r : recordList) controller.addMedicalRecord(r);
        // --- LOAD TO SERVICE FIX END ---
        loadBills();
        // --- LOAD TO SERVICE FIX START ---
        for (Bill b : billList) controller.createBill(b);
        // --- LOAD TO SERVICE FIX END ---
    }
    ...
    private void loadPatients() {
        patientList.clear();
        try (java.util.Scanner sc = new java.util.Scanner(new java.io.File(PATIENTS_FILE))) {
            while (sc.hasNextLine()) {
                ...
                if (parts.length >= 4) {
                    ...
                    boolean admitted = Boolean.parseBoolean(parts[3]);
                    Patient p = new Patient(id, name, dob);
                    // --- PERSISTENCE FIX ---
                    if (admitted) p.admitPatient();
                    // --- END PERSISTENCE FIX ---
                    patientList.add(p);
                }
            }
    ...
    private void savePatients() {
        ensureDataDir();
        try (java.io.PrintWriter pw = new java.io.PrintWriter(PATIENTS_FILE)) {
            for (Patient p : patientList) {
                pw.println(String.join(",",
                    p.getPatientId(),
                    p.getPatientName().replace(","," "),
                    p.getDateOfBirth().toString(),
                    Boolean.toString(p.isPatientAdmitted())
                ));
            }
        } catch (Exception ignored) {}
    }
    ...
    private void loadInventory() {
        inventoryList.clear();
        try (java.util.Scanner sc = new java.util.Scanner(new java.io.File(INVENTORY_FILE))) {
            while (sc.hasNextLine()) {
                String[] parts = sc.nextLine().split(",");
                if (parts.length >= 4) {
                    String id = parts[0];
    ...
    private void saveInventory() {
        ensureDataDir();
        try (java.io.PrintWriter pw = new java.io.PrintWriter(INVENTORY_FILE)) {
    ...
    private void loadAppointments() {
        ...
        try (java.util.Scanner sc = new java.util.Scanner(new java.io.File(APPOINTMENTS_FILE))) {
            while (sc.hasNextLine()) {
                String[] parts = sc.nextLine().split(",");
                if (parts.length >= 5) {
                    String id = parts[0];
                    ...
                    Patient patient = patientList.stream().filter(p -> p.getPatientId().equals(patientId)).findFirst().orElse(null);
                    if (patient != null) {
                        Appointment a = new Appointment(id, patient, dt, type);
                        // --- PERSISTENCE FIX ---
                        if ("COMPLETED".equals(status)) a.complete();
                        else if ("CANCELLED".equals(status)) a.cancel();
                        // --- END PERSISTENCE FIX ---
    ...
    private void saveAppointments() {
        ensureDataDir();
        try (java.io.PrintWriter pw = new java.io.PrintWriter(APPOINTMENTS_FILE)) {
    ...
    private void loadMedicalRecords() {
        recordList.clear();
        try (java.util.Scanner sc = new java.util.Scanner(new java.io.File(RECORDS_FILE))) {
            while (sc.hasNextLine()) {
                String[] parts = sc.nextLine().split(",");
                if (parts.length >= 6) {
                    String id = parts[0];
                    String patientId = parts[1];
                    String apptId = parts[2];
                    String diagnosis = parts[3];
                    String notes = parts[4];
                    LocalDate date = LocalDate.parse(parts[5]);
                    Patient patient = patientList.stream().filter(p -> p.getPatientId().equals(patientId)).findFirst().orElse(null);
                    Appointment appt = appointmentList.stream().filter(a -> a.getAppointmentId().equals(apptId)).findFirst().orElse(null);
                    if (patient != null && appt != null) {
                        MedicalRecord r = new MedicalRecord(id, patient, appt, diagnosis, notes, date);
                        recordList.add(r);
                    }
                }
            }
    ...
    private void saveMedicalRecords() {
        ensureDataDir();
        try (java.io.PrintWriter pw = new java.io.PrintWriter(RECORDS_FILE)) {
    ...
    private void loadBills() {
        billList.clear();
        try (java.util.Scanner sc = new java.util.Scanner(new java.io.File(BILLS_FILE))) {
            while (sc.hasNextLine()) {
                ...
                if (parts.length >= 5) {
                    ...
                    String lineItemsStr = parts[3];
                    double total = Double.parseDouble(parts[4]);
                    Patient patient = patientList.stream().filter(p -> p.getPatientId().equals(patientId)).findFirst().orElse(null);
                    if (patient != null) {
                        Bill b = new Bill(id, patient);
                        // --- PERSISTENCE FIX ---
                        if ("PAID".equals(status)) b.markAsPaid("LOADED");
                        // --- END PERSISTENCE FIX ---
                        // Parse line items
                        if (!lineItemsStr.isEmpty()) {
                            String[] items = lineItemsStr.split("\\|");
                            for (String item : items) {
                                String[] li = item.split("~");
                                if (li.length == 2) {
                                    b.addLineItem(new Bill.LineItem(li[0], Double.parseDouble(li[1])));
                                }
                            }
                        }
                        billList.add(b);
    ...
    private void saveBills() {
        ensureDataDir();
        try (java.io.PrintWriter pw = new java.io.PrintWriter(BILLS_FILE)) {
            for (Bill b : billList) {
                StringBuilder lineItems = new StringBuilder();
                for (Bill.LineItem li : b.getLineItems()) {
                    if (lineItems.length() > 0) lineItems.append("|");
                    lineItems.append(li.getDescription().replace("|"," ").replace("~"," ")).append("~").append(li.getAmount());
                }
                pw.println(String.join(",",
                    b.getBillId(),
                    ...
                    b.getStatus().name(),
                    lineItems.toString(),
                    Double.toString(b.getTotalAmount())
                ));
            }
        } catch (Exception ignored) {}
    }
    ...
    private void ensureDataDir() {
        ...
        if (!dir.exists()) {
            ...
            if (!created) {
                System.err.println("ERROR: Could not create data directory: " + DATA_DIR);
            }
        }
    }
    // --- DATA DIR FIX END ---

    // Call loadAllData() in the constructor (before mock data)
    public HospitalApp() {
        // --- ENHANCED MOCK DATA TO FILES START ---
        try {
            java.io.File pf = new java.io.File(PATIENTS_FILE);
            java.io.File af = new java.io.File(APPOINTMENTS_FILE);
            java.io.File rf = new java.io.File(RECORDS_FILE);
            java.io.File bf = new java.io.File(BILLS_FILE);
            java.io.File inf = new java.io.File(INVENTORY_FILE);
            ensureDataDir();
            if (pf.length() == 0) {
                try (java.io.PrintWriter pw = new java.io.PrintWriter(PATIENTS_FILE)) {
                    pw.println("P101,Alice Smith,1990-05-12,false");
                    pw.println("P102,Bob Johnson,1985-08-23,false");
                    pw.println("P103,Charlie Lee,1978-02-03,false");
                }
            }
            if (inf.length() == 0) {
                try (java.io.PrintWriter pw = new java.io.PrintWriter(INVENTORY_FILE)) {
                    pw.println("INV101,Aspirin,50,0.25");
                    pw.println("INV102,Bandage,100,0.10");
                    pw.println("INV103,Stethoscope,5,25.00");
                    pw.println("INV104,Antibiotic,30,1.50");
                }
            }
            if (af.length() == 0) {
                try (java.io.PrintWriter pw = new java.io.PrintWriter(APPOINTMENTS_FILE)) {
                    pw.println("A1001,P101,2025-04-25T10:00:00,Checkup,SCHEDULED");
                    pw.println("A1002,P102,2025-04-26T14:30:00,Consultation,SCHEDULED");
                    pw.println("A1003,P103,2025-04-27T09:15:00,Follow-up,SCHEDULED");
                }
            }
            if (rf.length() == 0) {
                try (java.io.PrintWriter pw = new java.io.PrintWriter(RECORDS_FILE)) {
                    pw.println("MR10001,P101,A1001,Flu,Prescribed rest and fluids.,2025-04-25");
                    pw.println("MR10002,P102,A1002,Sprained Ankle,Applied bandage. Advised ice and elevation.,2025-04-26");
                    pw.println("MR10003,P103,A1003,Checkup,All vitals normal.,2025-04-27");
                }
            }
            if (bf.length() == 0) {
                try (java.io.PrintWriter pw = new java.io.PrintWriter(BILLS_FILE)) {
                    pw.println("B101,P101,UNPAID,Consultation Fee~50.00|Medication~10.00,60.00");
                    pw.println("B102,P102,PAID,Consultation Fee~50.00|Bandage~5.00,55.00");
                    pw.println("B103,P103,UNPAID,Checkup~40.00,40.00");
                }
            }
        } catch (Exception e) {
            System.err.println("MOCK DATA FILE ERROR: " + e.getMessage());
        }
        // --- ENHANCED MOCK DATA TO FILES END ---
        loadAllData();
    }
    ...
    public void start(Stage primaryStage) {
        ...
        titleLabel.setStyle("-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: " + COLOR_DARK + ";");
        
        Label prompt = new Label("Select your role to continue");
        prompt.setStyle("-fx-font-size: 16px; -fx-text-fill: #666;");
        
        Button doctorBtn = new Button("Doctor");
        Button adminBtn = new Button("Admin");
        
        doctorBtn.setStyle(BUTTON_STYLE_PRIMARY);
        doctorBtn.setMinWidth(200);
        doctorBtn.setEffect(dropShadow);
        
        adminBtn.setStyle(BUTTON_STYLE_SECONDARY);
        ...
        VBox root = new VBox(20, titleLabel, prompt, doctorBtn, adminBtn);
        root.setStyle("-fx-alignment: center; -fx-padding: 40; " + BG_STYLE_LIGHT);
    ...
    private void showDashboard(Stage stage, String role) {
        ...
        dashLabel.setStyle(TITLE_STYLE);
        ...
        String dashButtonStyle = "-fx-min-width: 250px; -fx-padding: 15px; -fx-font-size: 16px; " +
                              "-fx-background-radius: 5; -fx-background-color: white; " +
        ...
        logoutBtn.setStyle("-fx-min-width: 120px; -fx-padding: 10px; -fx-background-color: " + 
                        COLOR_ACCENT + "; -fx-text-fill: white; -fx-background-radius: 5;");
        
        // Apply hover effects
        String hoverStyle = "-fx-background-color: " + COLOR_PRIMARY + "; -fx-text-fill: white;";
        
        patientBtn.setOnMouseEntered(e -> patientBtn.setStyle(dashButtonStyle + hoverStyle));
        patientBtn.setOnMouseExited(e -> patientBtn.setStyle(dashButtonStyle));
        
        apptBtn.setOnMouseEntered(e -> apptBtn.setStyle(dashButtonStyle + hoverStyle));
        apptBtn.setOnMouseExited(e -> apptBtn.setStyle(dashButtonStyle));
        
        recordBtn.setOnMouseEntered(e -> recordBtn.setStyle(dashButtonStyle + hoverStyle));
        recordBtn.setOnMouseExited(e -> recordBtn.setStyle(dashButtonStyle));
        
        billingBtn.setOnMouseEntered(e -> billingBtn.setStyle(dashButtonStyle + hoverStyle));
        billingBtn.setOnMouseExited(e -> billingBtn.setStyle(dashButtonStyle));
        
        inventoryBtn.setOnMouseEntered(e -> inventoryBtn.setStyle(dashButtonStyle + hoverStyle));
        inventoryBtn.setOnMouseExited(e -> inventoryBtn.setStyle(dashButtonStyle));
        ...
        apptBtn.setEffect(dropShadow);
        recordBtn.setEffect(dropShadow);
        billingBtn.setEffect(dropShadow);
        inventoryBtn.setEffect(dropShadow);
        logoutBtn.setEffect(dropShadow);

        VBox dash;
        if (role.equals("Admin")) {
            dash = new VBox(15, welcomeLabel, dashLabel, billingBtn, inventoryBtn, new Region(), logoutBtn);
        } else {
            dash = new VBox(15, welcomeLabel, dashLabel, patientBtn, apptBtn, recordBtn, new Region(), logoutBtn);
        }
        
        // Add spacer between content and logout
        Region spacer = (Region) dash.getChildren().get(dash.getChildren().size() - 2);
        VBox.setVgrow(spacer, Priority.ALWAYS);
        
        dash.setStyle("-fx-alignment: center; -fx-padding: 40; " + BG_STYLE_LIGHT);
        dash.setAlignment(Pos.TOP_CENTER);

        stage.setTitle("Hospital System - " + role + " Dashboard");
        stage.setScene(new Scene(dash, 500, 600));
    }
    ...
    private HBox createTopBar(String title, Stage stage, String role) {
        ...
        backBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: " + COLOR_PRIMARY + 
        ...
        HBox topBar = new HBox(15, backBtn, titleLabel, spacer, roleLabel);
        topBar.setAlignment(Pos.CENTER_LEFT);
        topBar.setPadding(new Insets(0, 0, 20, 0));
        topBar.setStyle("-fx-border-color: transparent transparent #e0e0e0 transparent; -fx-border-width: 0 0 1 0; -fx-padding: 0 0 10 0;");
    ...
    private HBox createSearchAddBar(String searchPrompt, String addTooltip, javafx.event.EventHandler<javafx.event.ActionEvent> addAction) {
        ...
        searchField.setStyle("-fx-background-radius: 20; -fx-padding: 8 15 8 35;");
        
        // Set an ID on the search field to make it easier to find
        searchField.setId("searchField");
        
        Label searchIcon = new Label("\uD83D\uDD0D");
        searchIcon.setStyle("-fx-text-fill: #999; -fx-font-size: 14px;");
        
        // Create a container for the search field and icon
        StackPane searchContainer = new StackPane();
        searchContainer.getChildren().add(searchField);
        
        // Position the search icon on the left side of the text field
        StackPane searchIconPane = new StackPane(searchIcon);
        searchIconPane.setAlignment(Pos.CENTER_LEFT);
        searchIconPane.setPadding(new Insets(0, 0, 0, 10));
        searchIconPane.setMouseTransparent(true); // Make sure clicks go through to the text field
        searchContainer.getChildren().add(searchIconPane);
        ...
        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);
        HBox searchAddRow = new HBox(15, searchContainer, spacer, addBtn);
        searchAddRow.setAlignment(Pos.CENTER_LEFT);
        searchAddRow.setPadding(new Insets(0, 0, 20, 0));
    ...
    private TextField getSearchFieldFromBar(HBox searchAddBar) {
        if (searchAddBar == null) {
            return null;
        }
        
        // First try to find by ID
        Node foundNode = searchAddBar.lookup("#searchField");
        if (foundNode instanceof TextField) {
            System.out.println("DEBUG: Found search field by ID");
            return (TextField) foundNode;
        }
        
        // If we can't find it by ID, look through children
        if (!searchAddBar.getChildren().isEmpty() && searchAddBar.getChildren().get(0) instanceof StackPane) {
            StackPane searchContainer = (StackPane) searchAddBar.getChildren().get(0);
            for (Node node : searchContainer.getChildren()) {
                if (node instanceof TextField) {
                    System.out.println("DEBUG: Found search field in container");
                    return (TextField) node;
                }
            }
        }
        
        System.out.println("DEBUG: Could not find search field");
        return null;
    }

    private <T> VBox createMainScreenLayout(HBox topBar, HBox searchAddBar, TableView<T> table) {
        // Create a container for the table with padding and styling
        VBox tableContainer = new VBox(table);
        tableContainer.setStyle("-fx-background-color: white; -fx-background-radius: 8; -fx-padding: 15;");
        tableContainer.setEffect(new DropShadow(10, Color.rgb(0, 0, 0, 0.1)));
        
        VBox tableBox = new VBox(10, searchAddBar, tableContainer);
        tableBox.setPadding(new Insets(0));
        
        VBox root = new VBox(15, topBar, tableBox);
        root.setPadding(new Insets(25));
        root.setStyle(BG_STYLE_LIGHT);
    ...
    private void showPatientManagement(Stage stage, String role) {
        HBox topBar = createTopBar("Patient Directory", stage, role);
        HBox searchAddRow = createSearchAddBar("Search patients by name or ID...", "Add a new patient", e -> showAddPatientDialog());
    ...
    private TableView<Patient> createPatientTable() {
        ...
        // --- CENTERING & FORMATTING FIX START ---
        String columnStyle = "-fx-alignment: CENTER; -fx-padding: 5;";
        // --- CENTERING & FORMATTING FIX END ---
        ...
        dobCol.setStyle(columnStyle);
        // Format LocalDate
        dobCol.setCellFactory(col -> new TableCell<Patient, LocalDate>() {
            @Override
        ...
        admittedCol.setStyle(columnStyle);
        admittedCol.setCellFactory(col -> new TableCell<Patient, String>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item);
                    setStyle("-fx-text-fill: " + (item.equals("Yes") ? COLOR_SECONDARY : "#999") + "; -fx-font-weight: bold; -fx-alignment: CENTER;");
                }
            }
        });
    ...
    private void filterPatientList(String searchText) {
        try {
            System.out.println("DEBUG: Filtering patient list with text: '" + searchText + "'");
    ...
    private void showPatientDetails(Patient patient) {
        ...
        dialog.setHeaderText("Patient: " + patient.getPatientName());
        
        // Create a styled content area
        VBox content = new VBox(15);
        content.setStyle("-fx-padding: 20;");
        
        // Add patient details with styled labels
        addDetailRow(content, "ID:", patient.getPatientId());
        addDetailRow(content, "Date of Birth:", patient.getDateOfBirth().toString());
        addDetailRow(content, "Age:", String.valueOf(patient.getPatientAge()));
        addDetailRow(content, "Admitted:", patient.isPatientAdmitted() ? "Yes" : "No");
        
        // Use a styled pane for the dialog
    ...
    // Helper method to add a detail row with consistent styling
    private void addDetailRow(VBox container, String label, String value) {
        ...
        HBox row = new HBox(15, labelNode, valueNode);
    ...
    private void showAddPatientDialog() {
        ...
        dialogPane.setHeaderText("Enter New Patient Details");
        dialogPane.setStyle("-fx-background-color: white;");
        Label headerLabel = new Label("Enter New Patient Details");
        headerLabel.setStyle(SUBTITLE_STYLE);
        dialogPane.setHeader(new StackPane(headerLabel));

        // Controls with improved styling
        TextField nameField = new TextField(); 
        nameField.setPromptText("Name");
        nameField.setStyle("-fx-padding: 8; -fx-background-radius: 4;");
        
        DatePicker dobPicker = new DatePicker(); 
        dobPicker.setPromptText("Date of Birth");
        dobPicker.setStyle("-fx-padding: 8;");
        ...
        VBox vbox = new VBox(8, nameLabel, nameField, dobLabel, dobPicker);
        ...
        okButton.setStyle("-fx-background-color: " + COLOR_PRIMARY + "; -fx-text-fill: white;");
        
        // Enable OK only when valid
        okButton.setDisable(true);
        nameField.textProperty().addListener((obs, oldV, newV) -> 
            okButton.setDisable(newV.trim().isEmpty() || dobPicker.getValue() == null));
        dobPicker.valueProperty().addListener((obs, oldV, newV) -> 
            okButton.setDisable(newV == null || nameField.getText().trim().isEmpty()));
    ...
    private TableView<Appointment> createAppointmentTable() {
        ...
        patientNameCol.setStyle(columnStyle);
        ...
        patientIdCol.setStyle(columnStyle);
        ...
        dateTimeCol.setPrefWidth(180);
        dateTimeCol.setStyle(columnStyle);
        // Format LocalDateTime
        dateTimeCol.setCellFactory(col -> new TableCell<Appointment, LocalDateTime>() {
            @Override
            protected void updateItem(LocalDateTime item, boolean empty) {
                super.updateItem(item, empty);
                setText(empty || item == null ? null : DATETIME_FORMAT.format(item));
            }
        });
    ...
    private void filterAppointmentList(String searchText) {
        try {
            System.out.println("DEBUG: Filtering appointment list with text: '" + searchText + "'");
    ...
    private void showAddAppointmentDialog() {
        ...
        // Set cell factory to display patient name
        patientCombo.setCellFactory(lv -> new ListCell<Patient>() {
            ...
            protected void updateItem(Patient patient, boolean empty) {
                ...
                if (empty || patient == null) {
                    ...
                    setText(patient.getPatientName() + " (ID: " + patient.getPatientId() + ")");
                }
            }
        });
        // Set button cell to display selected patient name
    ...
    private void showMedicalRecords(Stage stage, String role) {
     ...
     private TableView<MedicalRecord> createMedicalRecordTable() {
        ...
        apptIdCol.setStyle(columnStyle);
        ...
        // Format LocalDate
        dateCol.setCellFactory(col -> new TableCell<MedicalRecord, LocalDate>() {
            @Override
            protected void updateItem(LocalDate item, boolean empty) {
                super.updateItem(item, empty);
                setText(empty || item == null ? null : DATE_FORMAT.format(item));
        ...
        diagnosisCol.setStyle(columnStyle);
    ...
    private void filterMedicalRecordList(String searchText) {
        try {
            System.out.println("DEBUG: Filtering medical record list with text: '" + searchText + "'");
     ...
     private void showAddMedicalRecordDialog() {
        ...
        ComboBox<Appointment> appointmentCombo = new ComboBox<>(); 
        appointmentCombo.setPromptText("Select Related Appointment");
        appointmentCombo.setDisable(true); // Disabled until patient is selected
        
        // Set cell factory to display appointment details
        appointmentCombo.setCellFactory(lv -> new ListCell<Appointment>() {
            @Override
            protected void updateItem(Appointment appt, boolean empty) {
                ...
                if (empty || appt == null) {
                    ...
                    setText(appt.getAppointmentId() + " - " + appt.getDateTime().toLocalDate() + " - " + appt.getType());
                }
            }
        });
        // Set button cell to display selected appointment details
        appointmentCombo.setButtonCell(new ListCell<Appointment>() {
            @Override
            protected void updateItem(Appointment appt, boolean empty) {
                super.updateItem(appt, empty);
                if (empty || appt == null) {
        ...
        patientCombo.valueProperty().addListener((obs, oldPatient, newPatient) -> {
            if (newPatient != null) {
                ...
                if (patientAppointments.isEmpty()) {
                    ...
                    appointmentCombo.setDisable(false);
                    appointmentCombo.setPromptText("Select Related Appointment");
                }
            } else {
        ...
        if (patientList.isEmpty()) {
             showError("Cannot add record: No patients registered.");
             return;
        }
    ...
    private TableView<Bill> createBillingTable() {
        ...
        amountCol.setStyle(columnStyle);
    ...
    private void filterBillList(String searchText) {
        try {
            System.out.println("DEBUG: Filtering bill list with text: '" + searchText + "'");
    ...
    private void showInventory(Stage stage, String role) {
     ...
     private TableView<InventoryItem> createInventoryTable() {
        ...
        quantityCol.setStyle(columnStyle);
        ...
        TableColumn<InventoryItem, Double> totalValueCol = new TableColumn<>("Total Value");
        ...
        totalValueCol.setStyle(columnStyle);
        totalValueCol.setCellFactory(tc -> new TableCell<InventoryItem, Double>() {
            @Override
        ...
        table.getColumns().setAll(idCol, nameCol, quantityCol, priceCol, totalValueCol, actionsCol);
    ...
    private void refreshInventoryList() {
     ...
     private void filterInventoryList(String searchText) {
        try {
            System.out.println("DEBUG: Filtering inventory list with text: '" + searchText + "'");
    ...
    private void showError(String msg) {
        ...
        // Find the header label and style it
        Label contentText = (Label) dialogPane.lookup(".content.label");
        if (contentText != null) {
            contentText.setStyle("-fx-font-size: 14px; -fx-padding: 10px;");
        }
